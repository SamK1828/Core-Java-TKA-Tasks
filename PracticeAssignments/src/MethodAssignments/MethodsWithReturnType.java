...
public class MethodsWithReturnType {
	...
	public static String returnMultiplicationTable(int num) {
		String table = "Multiplication Table of " + num + ":\n";
	    for (int i = 1; i <= 10; i++) {
	        table += num + " x " + i + " = " + (num * i) + "\n";
	    }
	    return table;
	...
	public static String returnNumbers(int n) {
		String result = "Numbers from 1 to "+n+": ";
		for (int i = 1; i <= n; i++) {
			if (i == n) {
				result += i; // Add the last number without space
			} else {
				result += i + " "; // Add number with space
			}
		}
		return result;
	}
	...
	public static String returnReverseNumbers(int n) {
		String result = "Numbers from " + n + " to 1: ";
		for (int i = n; i >= 1; i--) {
			if (i == 1) {
				result += i; // Add the last number without space
			} else {
